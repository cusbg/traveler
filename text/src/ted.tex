...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    \Procedure {mapping}{$F, G, TreeDistance$}
      ...
      \State $Forests_{F} \gets$ \Call{Get Ordered Subforests}{$F, \sigma$}
      \State $Forests_{G} \gets$ \Call{Get Ordered Subforests}{$G, \sigma$}
      \While {$Forests_{F} \vee Forests_{G}$ nieje prazdny}
        \State $F' \gets \Call{get forest}{Forests_{F}}$
        \State $G' \gets \Call{get forest}{Forests_{G}}$
        \While {$F'$ nieje prazdny alebo $G'$ nieje prazdny les}
        \EndWhile
      \EndWhile
      \ForAll {$F' \in Forests_{F}$}
      \EndFor
    \EndProcedure
  \item[]
    \Procedure {get forest}{$Forests$}
